    ...
    <div className='overflow-hidden'>
      ...
      <div className="flex mt-7">
        ...
        <div className="m-3 w-[50%] overflow-hidden text-start">
          ...
          <div className='flex'>
            <p onClick={() => setIsPopupOpen(!isPopupOpen)} className='w-[700px] text-lg font-poppins cursor-pointer text-left overflow-hidden text-ellipsis whitespace-nowrap'>
      ...
      <div className='flex gap-10 mt-10 all-sections-wrapper'>
        {tabsObject.map((eachTab, i) => (
          <div key={eachTab.id}>
            <button className={`text-xl font-poppins  ${eachTab.id === activeTab ? 'font-bold pb-4 border-b-4 border-black' : ''}`} onClick={() => setActiveTab(tabsObject[i].id)}>{eachTab.text}</button>
          </div>
        ))}
      </div>
      ...
      {activeTab === 'HOME' && <div className='overflow-hidden'><ChannelHomeSection uploads={channelDetails?.items[0]?.contentDetails?.relatedPlaylists?.uploads} /></div>}
...
{/* <div className='w-[100%] flex flex-col items-center'>
  ...
  {activeTab === 'HOME' && <div className='w-[90%]'><ChannelHomeSection uploads={channelDetails?.items[0]?.contentDetails?.relatedPlaylists?.uploads} /></div>}
  {activeTab === 'VIDEOS' && <ChannelVideosSection />}
  {activeTab === 'SHORTS' && <ChannelShortsSection />}
</div>
 */}
